    ...
    Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colors.background) {
        ModalBottomSheetLayout(
            ...
            Scaffold(
                ...
                FindAccountScreen(
                    channels, countries, countryChoice,
                    navigateToAdd = {
...
fun TopBar(showingHelp: MutableState<Boolean>) {
    Column(modifier = Modifier.fillMaxWidth()) {
        CenterAlignedTopAppBar(
            ...
            actions = {
                IconButton(onClick = { showingHelp.value = true }) {
                    Icon(
...
fun showHelp(showingHelp: MutableState<Boolean>) {
    if (showingHelp.value) {
        AlertDialog(
            title = { Text(stringResource(R.string.add_accounts_help_title)) },
...
fun FindAccountScreen(
    ...
    Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.Center) {
        Row(Modifier.padding(horizontal = 8.dp)) {
            ...
            Column() {
                StaxTextField(
                    searchValue, R.string.search, R.drawable.ic_search,
                    onChange = {
                        searchValue = it
                        onSearch(it.text)
                    }
