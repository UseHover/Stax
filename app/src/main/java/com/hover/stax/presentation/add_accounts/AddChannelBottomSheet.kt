...
fun AddChannelBottomSheet(channel: Channel?, addAccountViewModel: AddAccountViewModel = getViewModel()) {
	...
	Column(modifier = Modifier
		.fillMaxWidth()
		.padding(bottom = 1.dp)
		.padding(horizontal = 13.dp), Arrangement.Bottom) {
		if (channel != null) {
...
fun SimNotPresentDetails(simChannels: List<Pair<SimInfo, Channel?>>, channel: Channel, error: MutableState<String>, addAccountViewModel: AddAccountViewModel) {
	...
	Text(text = stringResource(id = R.string.link_sim_error_details),
		modifier = Modifier.fillMaxWidth().padding(vertical = 13.dp))

	Text(text = stringResource(id = R.string.sims_detected),
		modifier = Modifier.fillMaxWidth().padding(vertical = 13.dp))
	...
	SecondaryButton(text = stringResource(R.string.save_without_sim_btn),
