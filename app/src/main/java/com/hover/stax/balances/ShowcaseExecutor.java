...
public class ShowcaseExecutor implements NavigationInterface {
    ...
    private void startShowcase(String head, String body, BubbleShowCaseListener listener, View view, BubbleShowCase.ArrowPosition arrowPosition, boolean shouldShowOnce) {
        try {
            if (shouldShowOnce)
                BubbleShowCase.Companion.showcaseOnce(head, body, arrowPosition, listener, view, activity);
            else
                BubbleShowCase.Companion.showCase(head, body, arrowPosition, listener, view, activity);
