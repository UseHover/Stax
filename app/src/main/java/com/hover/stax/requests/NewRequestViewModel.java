...
public class NewRequestViewModel extends AbstractFormViewModel {
    ...
    LiveData<String> getRequesterNumber() {
        if (requesterNumber == null) {
            requesterNumber = new MediatorLiveData<>();
        }
    ...
    void resetRecipients() {
        requestees.setValue(new ArrayList<>());
    }
    ...
    boolean validAmount() {
        return (amount.getValue() != null && !amount.getValue().isEmpty() && amount.getValue().matches("\\d+") && !amount.getValue().matches("[0]+"));
    }
    ...
    boolean validNote() {
        return (note.getValue() != null && !note.getValue().isEmpty());
    }
    ...
    LiveData<Request> getRequest() {
        if (formulatedRequest == null) {
            formulatedRequest = new MutableLiveData<>();
        }
    ...
    LiveData<List<Request>> getRequests() {
        if (finalRequests == null) {
            finalRequests = new MutableLiveData<>();
        }
