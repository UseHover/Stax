...
class LoginViewModel(val repo: DatabaseRepo, val application: Application) : ViewModel() {
	...
	private fun uploadUserToStax(email: String?, token: String?) {
		if (getUsername().isNullOrEmpty() && !email.isNullOrEmpty()) {
			...
			viewModelScope.launch(Dispatchers.IO) {
				try {
					val result =
						LoginNetworking(application).uploadUserToStax(email, optedIn.value!!, token)
					Timber.e("Uploading user to stax came back: ${result.code}")

					if (result.code in 200..299) onSuccess(JSONObject(result.body!!.string()),
						application.getString(R.string.uploaded_to_hover,
	...
	fun saveReferee(refereeCode: String, name: String, phone: String) {
		viewModelScope.launch(Dispatchers.IO) {
			if (!email.value.isNullOrEmpty()) {
				...
				try {
					val result = LoginNetworking(application).uploadReferee(email.value!!,
						refereeCode,
						name,
						phone,
						account?.idToken)
	...
	private fun onSuccess(json: JSONObject, name: String) {
		...
		refereeCode.value?.let {
			...
			try {
				...
				AnalyticsUtil.logAnalyticsEvent(application.getString(R.string.upload_referee_success_event),
					data,
					application)
			} catch (e: Exception) {
	...
	private fun getEmail(): String? {
		if (Utils.getString(EMAIL, application) == null && Utils.getString(BOUNTY_EMAIL_KEY,
				application) != null) {
	...
	private fun onError(message: String) {
		AnalyticsUtil.logErrorAndReportToFirebase(LoginViewModel::class.java.simpleName,
			message,
			null)
