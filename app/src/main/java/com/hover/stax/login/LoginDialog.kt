...
class LoginDialog : DialogFragment(), View.OnClickListener {
	...
	override fun onClick(v: View?) {
		...
		if (networkMonitor.isNetworkConnected) {
			AnalyticsUtil.logAnalyticsEvent(getString(R.string.clicked_bounty_email_continue_btn),
				requireContext())
			updateProgress(0)
			startActivityForResult(loginViewModel.signInClient.signInIntent,
	...
	override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
		...
		if (requestCode == AbstractGoogleAuthActivity.LOGIN_REQUEST) loginViewModel.signIntoFirebaseAsync(
			data,
