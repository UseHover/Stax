...
class MessagesAdapter internal constructor(private val messagesList: List<UssdCallResponse>?,
	...
	override fun onCreateViewHolder(parent: ViewGroup,
		...
		val binding = TransactionMessagesItemsBinding.inflate(LayoutInflater.from(parent.context),
			parent,
			false)
	...
	private fun setEnteredValueView(model: UssdCallResponse, holder: TransactionMessageViewHolder) {
		if (model.enteredValue.isNotEmpty()) {
			...
			if (model.isShortCode) styleAsLink(holder.binding.messageEnteredValue,
				model.enteredValue)
		}
