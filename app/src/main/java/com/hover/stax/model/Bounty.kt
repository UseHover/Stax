...
class Bounty(val action: HoverAction, val transactions: List<StaxTransaction>) {
    ...
    fun generateDescription(c: Context): String = when (action.transaction_type) {
        ...
        HoverAction.BILL -> c.getString(
            R.string.descrip_bounty_bill,
            if (action.isOnNetwork) "" else c.getString(
                R.string.descrip_bounty_c2b_b, action.to_institution_name
            )
        )
    ...
    fun getInstructions(c: Context): String = when (action.transaction_type) {
        ...
        HoverAction.BILL -> c.getString(
            R.string.bounty_bill_explain,
            if (action.isOnNetwork) c.getString(
                R.string.bounty_bill_any
