...
object PhoneHelper {
    ...
    private fun convertToCountry(num: String, from_country: String, to_country: String): String {
        ...
        try {
            ...
            number =
                phoneUtil.formatNumberForMobileDialing(phone, from_country, false).replace("+", "")
...
//            logErrorAndReportToFirebase(TAG, "Failed to transform number for contact; doing it the old fashioned way.", e)
            if (number.startsWith("+")) number.substring(4) else if (number.startsWith("0")) number.substring(
                1
            ) else number
        }
    }
