...
data class StaxTransaction(
    ...
    constructor(data: Intent, action: HoverAction, contact: StaxContact?, context: Context) : this(
        ...
        if (hasExtra(data, TransactionContract.COLUMN_CATEGORY)) data.getStringExtra(
            TransactionContract.COLUMN_CATEGORY
        )!! else "started",
    ...
    private fun parseExtras(extras: HashMap<String, String>?) {
        ...
        if (extras.containsKey(HoverAction.AMOUNT_KEY)) amount =
            Utils.amountToDouble(extras[HoverAction.AMOUNT_KEY]!!)
        ...
        if (extras.containsKey(HoverAction.ACCOUNT_KEY)) counterpartyNo =
            extras[HoverAction.ACCOUNT_KEY]
    ...
    private fun generateDescription(
        ...
        return when (transaction_type) {
            ...
            HoverAction.MERCHANT -> c.getString(
                R.string.descrip_merchant_paid,
                amountStr,
                counterpartyNo
            )
