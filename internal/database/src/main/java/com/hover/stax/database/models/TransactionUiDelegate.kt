 ...
 */
package com.hover.stax.database.models
...
interface TransactionUiDelegate {
    ...
    fun humanCategory(c: Context): String {
        return if (transaction.isFailed) transaction.category.replace("-", " ")
    ...
    fun title(c: Context): String {
        var str =
            if (transaction.isBalanceType && transaction.isSuccessful && !transaction.isRecorded)
    ...
    fun shortStatusExplain(action: HoverAction?, institutionName: String, c: Context): String {
        ...
        return when {
            ...
            transaction.status == Transaction.PENDING -> c.getString(
                R.string.pending_cardhead_with_isntType,
                institutionName
            )

            transaction.status == Transaction.SUCCEEDED && transaction.balance.isNullOrEmpty() -> c.getString(
                R.string.successful_label
            )
    ...
    private fun shortFailureMessage(a: HoverAction?, c: Context): String {
        return when (transaction.category) {
            ...
            StaxTransaction.INCOMPLETE_ERROR -> c.getString(
                R.string.incomplete_error_short,
                getServiceName(a, c)
            )

            StaxTransaction.NO_RESPONSE_ERROR -> c.getString(
                R.string.no_response_error_short,
                getServiceName(a, c)
            )
    ...
    private fun longFailureMessage(a: HoverAction?, c: Context): String {
        return when (transaction.category) {
            StaxTransaction.BALANCE_ERROR -> c.getString(
            ...
            StaxTransaction.UNREGISTERED_ERROR -> c.getString(
                R.string.unregistered_error,
                getServiceName(a, c)
            )

            StaxTransaction.INCOMPLETE_ERROR -> c.getString(
                R.string.incomplete_desc,
                getServiceName(a, c)
            )

            StaxTransaction.NO_RESPONSE_ERROR -> c.getString(
                R.string.no_response_desc,
                getServiceName(a, c)
            )
    ...
    private fun lookupSuccessDescription(
        ...
        return if (!sms.isNullOrEmpty())
            sms.sortedByDescending { it.responseMessage.length }.map { it.responseMessage }
                .toString()
    ...
    val displayBalance: String
        get() = if (Utils.amountToDouble(transaction.balance) != null) Utils.formatAmount(
            transaction.balance
        ) else transaction.balance.toString()
}